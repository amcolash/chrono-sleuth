...
export class DebugTool extends Dialog {
  ...
  create() {
    ...
    const items = this.makeTab('Items', 0);
    const journal = this.makeTab('Journal', 1);
    const quests = this.makeTab('Quests', 2);
    ...
    this.tabs = [items, journal, quests];
