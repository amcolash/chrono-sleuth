...
Stats.Panel = function (name, fg, bg) {
  ...
  return {
    ...
    update: function (value) {
      // change to slowly decrease the min/max values back to zero
      min = Math.min(min * 0.99, value);
      max = Math.max(max * 0.99, value);
...
function createStats(game: Phaser.Game) {
  ...
  const renderPanel = stats.addPanel(new Stats.Panel('Render', '#ff8', '#221'));
  const stepPanel = stats.addPanel(new Stats.Panel('Step', '#8ff', '#212'));
  ...
  game.events.on(Phaser.Core.Events.POST_STEP, () => {
    stepPanel.update(performance.now() - step, 100);
  });
  ...
  game.events.on(Phaser.Core.Events.POST_RENDER, () => {
    renderPanel.update(performance.now() - render, 100);
    stats.end();
  });
}
